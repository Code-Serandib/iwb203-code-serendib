...
public type InfluenceIndexResult record {
    json details;
};
...
function GetDefaultPowerByStakeholderType(string sh_type) returns float|error {
    match sh_type {
        ...
        _ => {
            return error InvalidTypeException("\"" + sh_type + "\" is invalid type!...");
        }
    }
};
...
public function stakeholder_influence_index(
        ...
        float? legitimacy,
        float? urgency,
        stakeholderType? stakeholder_type) returns InfluenceIndexResult|error {
